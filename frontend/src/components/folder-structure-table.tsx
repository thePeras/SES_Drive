...
import { Download, Eye, Folder, Share2, Trash2, ChevronDown, User } from 'lucide-react';
...
export function FileTable({ files, onViewFile, onEnterDirectory, onShareFile, isSharedView = false }: FileTableProps) {
  return (
          ...
          <TableBody>
            {files.length === 0 ? (
                <TableRow>
                  ...
                  >
                    {isSharedView ? 'No files have been shared with you yet.' : 'No files found.'}
                  </TableCell>
                </TableRow>
            ) : (
                files.map((file, index) => (
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      ...
                      {file.permission === 'read-write' ? 'Read/Write' : 'Read Only'}
                    </span>
                          </TableCell>
                      )}
